...
serve(async (req) => {
  ...
  return rateLimitHandler(req, async (req) => {
    try {
      ...
      // Call BOG save card API
      const res = await fetch(`https://api.bog.ge/payments/v1/orders/${orderId}/cards`, {
        method: 'PUT',
        headers: {
      ...
      if (!res.ok) {
        ...
        throw new Error(`BOG save card failed: ${res.status} ${text}`);
      }
